...
| **Alpha** (`alpha`) | Basis points used to calculate PNK locked per draw. Formula: `lockedPNK = minStake × alpha / 10000`. This is locked from the juror's total stake, not necessarily equal to their total staked amount. | 10000 |
...
### hiddenVotes and Commit-Reveal

The `hiddenVotes` flag determines whether a court uses commit-reveal voting:

<Warning>
**General Court (ID 1) has `hiddenVotes = true`** — it uses commit-reveal. Jurors must submit a vote commitment hash first, then reveal their actual vote in a second transaction. Failing to reveal results in losing locked PNK.

Most specialized courts have `hiddenVotes = false` and use direct single-transaction voting. Always check the `hiddenVotes` column in the table below before staking in a court.
</Warning>
...
The `alpha` parameter determines how much PNK is locked **per draw** (per vote ID assigned to you):
...
<Warning>
**`alpha` operates on `minStake`, not on your total staked amount.** A juror can stake far more than `minStake` — that increases their drawing probability, but the locked amount per draw is always `minStake × alpha / 10000` regardless of how much extra they staked.

Example: Court has `minStake = 2,300 PNK`, `alpha = 10000`. A juror stakes 50,000 PNK.
- Drawing probability: proportional to 50,000 PNK in the sortition tree
- Locked amount per draw: `2,300 × 10000 / 10000 = 2,300 PNK` (not 50,000)
</Warning>

<Accordion title="Alpha Example">
**General Court: minStake = 2,300 PNK, alpha = 10000**

Locked amount per vote ID = 2,300 PNK

**Automated Curation Court (ID 31): minStake = 2,600 PNK, alpha = 290 (2.9%)**

Locked amount per vote ID = `2,600 × 290 / 10000 = ~75.4 PNK` — dramatically lower risk per draw, reflecting its low-stakes automated use case.
</Accordion>
