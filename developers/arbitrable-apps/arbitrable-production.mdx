...
| 5+ | Specialized | See [Court Hierarchy](/court/court-hierarchy) for the full 32-court table |

<Warning>
Court IDs on Arbitrum Sepolia (testnet) differ from Arbitrum One (mainnet) — the testnet only has 6 courts. Always verify IDs from the [kleros-v2 deployments](https://github.com/kleros/kleros-v2/tree/master/contracts/deployments) for your target network.
</Warning>
...
- [ ] If paying in ERC20: do **NOT** send ETH with `createDispute()` — KlerosCore uses a `NON_PAYABLE_AMOUNT` guard constant that will cause the call to revert if any ETH value is included when an ERC-20 fee token is specified
...
|-------|--------------------------|-------|
| Evidence | ~3.25 days (280,800 s) | Either party can submit evidence |
| Commit | ~6.75 days (583,200 s) | Jurors submit vote commitments (hidden-vote courts only) |
| Vote / Reveal | ~6.75 days (583,200 s) | Jurors reveal votes or cast direct votes |
| Appeal | ~4.5 days (388,800 s) | Anyone can fund an appeal |
| **Total (no appeal)** | **~21 days** | Minimum time to ruling (General Court) |
| **Per appeal round** | **+~21 days** | Each round adds roughly the same |

<Warning>
These are the **actual on-chain values** for the General Court on Arbitrum One as configured at deployment. Court parameters are governance-controlled and may change. Always query `courts(courtID)` on KlerosCore rather than hardcoding timing assumptions.
</Warning>
...
Kleros V2 is live on Arbitrum One. These items are specific to the current production deployment:

- [ ] **Contract upgrades:** V2 contracts use UUPS proxy patterns. Subscribe to the [Kleros blog](https://blog.kleros.io) and [Discord](https://discord.gg/kleros) for upgrade announcements.
- [ ] **Subgraph stability:** The V2 subgraph schema may change. Pin to a specific subgraph version and test after upgrades.
- [ ] **SDK status:** The `@kleros/kleros-v2-sdk` package is under active development. For production use, interact with contracts directly via ABIs from `@kleros/kleros-v2-contracts` or the [kleros-v2 repository](https://github.com/kleros/kleros-v2).
...
See [Deployment Addresses](/reference/contracts/deployment-addresses) for the full list including all contract implementations, gateways, and test tokens.

```solidity
// Querying court parameters on-chain
IKlerosCore core = IKlerosCore(0x991d2df165670b9cac3B022f4B68D65b664222ea);
...
<Warning>
Always verify addresses from the [official repository](https://github.com/kleros/kleros-v2/tree/master/contracts/deployments) before mainnet deployment. Proxy addresses are stable across upgrades but confirm before deploying.
</Warning>
