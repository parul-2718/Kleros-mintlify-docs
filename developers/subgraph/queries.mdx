...
description: "Common GraphQL query examples for Kleros V2 subgraphs to fetch courts, disputes, jurors, rounds, votes, evidence, and dispute templates on Arbitrum."
---
...
<Note>
Always use the **decentralized network** endpoint for production queries. Get the subgraph ID from the Studio pages listed in [Subgraph Endpoints](/developers/subgraph/endpoints) and construct the URL as `https://gateway.thegraph.com/api/{API_KEY}/subgraphs/id/{SUBGRAPH_ID}`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Light Curate Integration" icon="list-check" href="/developers/products/curate/light-curate">
