...
<Note>
The current production deployment on Arbitrum One is the "Neo" variant (`KlerosCore` version `0.10.0`, backed by `KlerosCoreBase`). A separate `KlerosCoreRuler` contract is also deployed on mainnet — it allows manual ruling overrides and is used as a governance/emergency tool, not for regular disputes. See [Deployment Addresses](/reference/contracts/deployment-addresses) for both addresses.
</Note>
...
Gets current ruling state. All three return values are required for correct arbitrable implementation:
- `ruling` — the current winning choice (0 = refuse to arbitrate)
- `tied` — `true` if multiple choices share the highest vote count; ruling defaults to 0 when tied
- `overridden` — `true` if the ruling was manually overridden (only possible via KlerosCoreRuler)

<Warning>
Arbitrable contracts must handle `tied = true`. In DisputeKitClassic, a tied vote results in `ruling = 0` (refuse to arbitrate). If your contract does not explicitly handle ruling 0, a tie will silently trigger your default/refund path.
</Warning>
...
Each dispute round stores a snapshot of the court parameters **at the time the round was created**. This means governance changes to court parameters (fees, juror counts, periods) do not retroactively affect in-progress rounds — only new rounds use updated values.

Key round fields:
- `drawnJurors[]` — array of drawn juror addresses (same address can appear multiple times)
- `drawnJurorFromCourtIDs[]` — tracks which sub-court each juror was originally staked in at draw time (used for reward routing in parent-court draws)
- `hiddenVotes`, `jurorsForCourtJump`, `timesPerPeriod` — copied from court at round creation
